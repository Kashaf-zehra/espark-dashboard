...
const AttendanceViewModal = ({ modalData, empData, handleClose }) => {
  function formatDate(inputDate) {
    ...
    const newDate = dayjs(formattedDate).format('MMMM DD, YYYY');
    if (newDate === 'Invalid Date') {
      return dayjs(inputDate).format('MMMM DD, YYYY');
    }
  ...
  const employeeData = [
    [
      ...
      ['Company:', 'eSpark talent', 'company_name'],
        ...
        {attendanceData?.map((data, dataIndex) => {
          ...
          return (
            <EmployeeRequestDetailRow
              ...
              value={
                data[3] === 'date'
                  ? formatDate(modalData[data[2]]) ||
                    formatDate(empData[data[2]])
                  : modalData[data[2]] || empData[data[2]]
                // preetyTime(modalData[data[2]])
              }
            />
          );
        })}
